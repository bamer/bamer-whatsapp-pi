...
export class MenuHandler {
	...
	private async manageAllowList(ctx: ExtensionCommandContext) {
		const list = this.sortContactsAlphabetically(
			this.sessionManager.getAllowList(),
		);
		const title = t("menu.allowed.title");
		const addNumberLabel = t("menu.allowed.addNumber");
		const backLabel = t("menu.root.back");
		const options = [
			...list.map((contact) => this.formatAllowedContactOption(contact)),
			addNumberLabel,
			backLabel,
		];
		...
		const selectedContact = list.find(
			(contact) => this.formatAllowedContactOption(contact) === choice,
		);
		if (!selectedContact) {
			await this.manageAllowList(ctx);
			return;
	...
	private async manageAllowedContact(
		...
		if (choice === removeSendNumberLabel) {
			...
			await this.manageAllowedContact(ctx, {
				...contact,
		...
		if (choice === addAliasLabel) {
			...
			await this.sessionManager.setAllowedContactAlias(
				contact.number,
				trimmedAlias,
			);
			ctx.ui.notify(
	...
	private async manageAllowedGroups(ctx: ExtensionCommandContext) {
		...
		const selectedGroup = list.find(
			(group) => this.formatAllowedGroupOption(group) === choice,
		);
		if (!selectedGroup) {
			await this.manageAllowedGroups(ctx);
			return;
	...
	private async manageAllowedGroup(
		ctx: ExtensionCommandContext,
		group: Contact,
	) {
		...
		if (choice === addAliasLabel) {
			...
			await this.sessionManager.setAllowedGroupAlias(
				group.number,
				trimmedAlias,
			);
			ctx.ui.notify(
	...
	private async manageUpdateList(ctx: ExtensionCommandContext) {
		...
		const selectedContact = list.find(
			(contact) => this.formatUpdateTargetOption(contact) === choice,
		);
		if (!selectedContact) {
			await this.manageUpdateList(ctx);
			return;
	...
	private async manageUpdateTarget(
		ctx: ExtensionCommandContext,
		contact: Contact,
	) {
	...
	private async manageRecents(ctx: ExtensionCommandContext) {
		const recentConversations =
			await this.recentsService.getRecentConversations();
		...
		for (let page = 0; page * pageSize < groupedEntries.length; ) {
			...
			const selectedEntry = pageEntries.find(
				(entry) => this.formatGroupedRecentOption(entry) === choice,
			);
	...
	private async manageRecentConversation(
		ctx: ExtensionCommandContext,
		...
		if (choice === allowContactLabel) {
			if (
				this.sessionManager.isConversationAllowed(conversation.senderNumber)
			) {
				ctx.ui.notify(
				...
				await this.sessionManager.addAllowedGroup(
				...
				await this.sessionManager.addNumber(
					conversation.senderNumber,
					conversation.senderName,
				);
				ctx.ui.notify(
		...
		if (choice === addToUpdateListLabel) {
			if (
				...
				await this.sessionManager.addUpdateNumber(
					conversation.senderNumber,
					conversation.senderName,
				);
				ctx.ui.notify(
		...
		if (choice === removeAliasLabel) {
			if (isGroup) {
				await this.sessionManager.removeAllowedGroupAlias(
					conversation.senderNumber,
				);
			} else {
				await this.sessionManager.removeAllowedContactAlias(
					conversation.senderNumber,
				);
			}
			ctx.ui.notify(
	...
	private async sendMessageToAllowedContact(
		ctx: ExtensionCommandContext,
		contact: Contact,
	) {
	...
	private async sendMessageToAllowedGroup(
		ctx: ExtensionCommandContext,
		group: Contact,
	) {
		await this.sendPromptedMenuMessage(ctx, {
	...
	private async showConversationHistoryForContact(
		...
		const history =
			await this.recentsService.getConversationHistory(senderNumber);
		...
		for (let page = 0; page * pageSize < sortedHistory.length; ) {
			...
			const selectedMessage = this.resolveHistorySelection(
				choice,
				historyOptions,
			);
			if (!selectedMessage) {
	...
	private buildHistoryOptions(
		history: RecentConversationMessage[],
	): HistoryOptionEntry[] {
		return history.map((message) => ({
			label: this.formatHistoryOption(
				message.timestamp,
				message.direction,
				message.text,
			),
	...
	private resolveHistorySelection(
		choice: string,
	...
	private groupRecentConversations(
		conversations: RecentConversationSummary[],
	): GroupedRecentEntry[] {
	...
	private formatRecentConversationOption(
		conversation: RecentConversationSummary,
	): string {
		const displayName = this.getConversationDisplayName(conversation);
	...
	private sortHistoryByMostRecent<T extends { timestamp: number }>(
		history: T[],
	): T[] {
		return [...history].sort((left, right) => {
			const dayComparison =
				this.getDayStart(right.timestamp) - this.getDayStart(left.timestamp);
			if (dayComparison !== 0) {
				return dayComparison;
			}

			return (
				this.getTimeOfDay(right.timestamp) - this.getTimeOfDay(left.timestamp)
			);
		});
	}
	...
	private formatHistoryOption(
		...
		const displayText =
			this.truncate(text, 60) || t("menu.recents.history.noText");
